<#
.SYNOPSIS
Task 7.2 contract suite: resource-group budget appears correctly in terraform plan.

.DESCRIPTION
Runs a non-interactive terraform plan and asserts Task 7.2 budget wiring:
Monthly resource-group budget resource exists, uses Task 7.1 variables through plan
values, contains both Forecasted and Actual notification thresholds, uses contact_roles
without hardcoded emails/groups, and keeps Task 6 shutdown schedule intact.

.NOTES
Contract tests encode docs/specs/task-7/task-7-budget-alerts-plan.md Task 7.2 acceptance.
...
function Assert-TerraformInvocationSucceeded {
  ...
  if ($Result.ExitCode -ne 0) {
    ...
    throw "Task 7.2 contract: expected $OperationDescription $SuccessPhrase. Exit code $($Result.ExitCode). Redacted output:`n$redacted"
  }
}
...
function Assert-PlanTextContains {
  ...
  if ($PlanText.IndexOf($RequiredSubstring, [System.StringComparison]::Ordinal) -lt 0) {
    throw "Task 7.2 contract: expected plan to contain $AssertionLabel. Missing literal: $RequiredSubstring"
  }
}
...
function Assert-PlanTextExcludesForbiddenSubstrings {
  ...
  foreach ($pattern in $forbiddenPatterns) {
    if ($PlanText.IndexOf($pattern, [System.StringComparison]::OrdinalIgnoreCase) -ge 0) {
      throw "Task 7.2 contract: plan must not include forbidden pattern '$pattern' in committed budget wiring."
    }
  }
}
...
function Get-BudgetResourcePlanBlock {
  ...
  if (-not $budgetBlockMatch.Success) {
    throw "Task 7.2 contract: unable to isolate budget resource plan block."
  }
...
function Assert-ExactlyOneBudgetResourceInPlan {
  ...
  if ($budgetResourceMatchCount -ne 1) {
    throw "Task 7.2 contract: expected exactly one azurerm_consumption_budget_resource_group resource in plan output, found $budgetResourceMatchCount."
  }
}
...
$script:ValidSshPublicKey = "ssh-ed25519 AAAAC3NzaC1lZDI1NTE5AAAAIEuSlvxTWf2H0tLCtLkM3PQWmZAnOEkjBLdyVAKDL43z task7-2-validation"

$previousTfVar = $env:TF_VAR_vm_admin_ssh_public_key
try {
  $env:TF_VAR_vm_admin_ssh_public_key = $script:ValidSshPublicKey

  Write-Host "Task 7.2 test: terraform plan should succeed with SSH key from environment."
  $planResult = Invoke-TerraformInfraCommand $script:Task72PlanHead
  ...
  Write-Host "Task 7.2 test: terraform validate should pass."
...
Write-Host "Task 7.2 budget resource plan contract test suite passed."
